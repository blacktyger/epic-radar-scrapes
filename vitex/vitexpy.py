...
class Token:
    ...
    def __repr__(self):
        return f"Token({self.symbol if self.symbol else self.id})"
...
class Address:
    """
    Manage Vitex addresses as Address objects, handle validations
    :param address: _str, Vite Chain address, example: 'vite_15d3230e3c31c009c968beea7160ae98b491475236ae2cddbc'
    """
    ...
    def address(self, value):
        value = value.lower()
        if value.startswith('vite_') and len(value) == 55:
            self._address = value
        else:
            raise ValueError(f"input: [{value}] is not a valid vite address")
...
class PublicAPI:
    ...
    def get_token(self, token: Union[Token, str]) -> list:
        """
        ...
        # Some parsing handlers for lookup convenience
        ...
        if isinstance(response, dict):
            if 'data' in response.keys() and 'ok' in response['msg']:
                response['msg'] = f'found token [{token.symbol if token.symbol else token.id}] but no data present'
                return [response]
            else:
                return self._create_token_object(response)
        else:
    ...
    def get_listed_tokens(self, quote_token: Union[Token, str]) -> list:
        """
        :param quote_token: REQUIRED! Quote token symbol. For example EPIC-002
        ...
        if not isinstance(quote_token, Token):
            quote_token = Token(symbol=quote_token)

        params = {'quoteTokenSymbol': quote_token.symbol}
        json_response = requests.get(url, params).json()
        response = self._response_parser(json_response)
        return self._create_token_object(response)

    def get_unlisted_tokens(self, quote_token: Union[Token, str]) -> list:
        """
        :param quote_token: REQUIRED! Quote token symbol. For example EPIC-002
    ...
    def get_trading_pair(self, pair: Union[TradingPair, str]) -> list:
        """
        :param pair: REQUIRED! Trading pair name. For example EPIC-002_BTC-000
    ...
    def get_order(self, address: Union[Address, str], orderId: str) -> list:
        """
    ...
    def get_orders(self, address: Union[Address, str], **kwargs) -> list:
        """
        :param address: REQUIRED! User's account address (not delegation address)
        ...
        :param status:Order status, valid in [ 0-10 ]. 3 , 5 - returns orders that
                    are unfilled or partially filled; 7 , 8 - returns orders
    ...
    def get_24hr_ticker_price_changes(self, quote_token: Union[Token, str] = None) -> list:
        """
        :param quote_token: Quote token object or symbol, example 'EPIC-002'. Returns all pairs if not present
    ...
    def get_order_book_ticker(self, pair: Union[TradingPair, str]) -> dict:
        """
        :param pair: REQUIRED! TradingPair object or string with symbol, example: EPIC-002_BTC-000
    ...
    def get_trade_history(self, pair: Union[TradingPair, str], **kwargs) -> list:
        """
        :param pair: REQUIRED! Trading pair name. For example. EPIC-002_BTC-000
        ...
        params = {**{'symbol': pair.symbol}, **kwargs}
        json_response = requests.get(url, params).json()
        response = self._response_parser(json_response)
        return response

    def get_detailed_trade_history(self, pair: Union[TradingPair, str], **kwargs) -> dict:
        """
        :param pair: REQUIRED! Trading pair name. For example, EPIC-002_BTC-000
    ...
    def get_order_book_depth(self, pair: Union[TradingPair, str], **kwargs) -> dict:
        """
        :param pair:  REQUIRED! Trading pair name. For example, EPIC-002_BTC-000
    ...
    def get_candlestick_bars(self, pair: Union[TradingPair, str], interval, **kwargs) -> dict:
        """
        :param pair: REQUIRED! Trading pair name. For example, GRIN-000_VITE
        ...
        params = {**{'symbol': pair.symbol, 'interval': interval}, **kwargs}
        json_response = requests.get(url, params).json()
        response = self._response_parser(json_response)
        return response

    def get_deposit_withdrawal_records(self, address: Union[Address, str],
                                       token: Union[Token, str], **kwargs) -> dict:
        """
        :param address: REQUIRED! Account address
        :param token: REQUIRED! Token id. For example tti_f370fadb275bc2a1a839c753
        :param offset:Search starting index, starts at 0 , default 0
        :param limit: Search limit, max 100 , default 100
        ...
        params = {**{'address': address, 'tokenId': token.id}, **kwargs}
        json_response = requests.get(url, params).json()
        response = self._response_parser(json_response)
        return response

    def get_currency_price(self, tokens: Union[list, str, Token], **kwargs) -> dict:
        """
        :param tokens: Trading pairs, split by ",". For example BTC-000,EPIC-002
        ...
        for token in tokens:
            if isinstance(tokens, Token):
                t_list.append(token.symbol)
            else:
                t_list.append(Token(token).symbol)

        tokens = ','.join(t_list)
        params = {**{'tokenSymbols': tokens}, **kwargs}
        json_response = requests.get(url, params).json()
        response = self._response_parser(json_response)
        return response

    def get_account_exchange_balance(self, address: Union[Address, str]) -> dict:
        """
        :param address: REQUIRED! Account address
